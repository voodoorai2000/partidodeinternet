...
#        debugger
        ...
        child_model = key.to_s.to_model || key.to_s.to_relation_model
        if child_model && child_model.name != 'Email'
  ...
  def shouldify(should_or_not)
    ...
    should_or_not.match(/not/) ? :should_not : :should
  end
  ...
  def split_and_strip(text, separator=",")
    text ? text.gsub(" and ", separator).split(separator).map(&:strip) : []
  end
  
  #para un belongs_to
  def last_mentioned_should_have_parent(parent, name, field="name")
    if parent_model = parent.to_model
      parent = parent_model.send("find_by_#{field}", name)
      (last_mentioned.send parent_model.name.underscore).should == parent
    else
      raise ModelNotMapped.new(parent)
    end
  end
  
  def add_last_resource_from_database_of(modelo)
    model = modelo.to_unquoted.to_model
    if resource = model.send("last")
      pile_up resource
    else
      NotFoundInDatabase.new(model, name)
    end
  ...
  def pile_up(mentioned)
    @resources ||= []
    #undefined method `updated_at' for #<Array:0x380e154> (NoMethodError)
    #de momento quito el if y los metemos todos
    #if mentioned != last_mentioned
      mentioned.class.send :include, Mencionado
      @resources.unshift mentioned
    #end
  ...
  #CUIDDDDAAAAOOE ESTO HA CAMBIAO (Mencionado)
  #def pile_up(mentioned)
  #  @resources ||= []
  #  #undefined method `updated_at' for #<Array:0x380e154> (NoMethodError)
  #  #de momento quito el if y los metemos todos
  #  #if last_mentioned.nil? || #si no hay recursos guardados lo guardamos
  #  #   (mentioned.updated_at != last_mentioned.updated_at) #si el recurso ha cambiado lo guardamos
